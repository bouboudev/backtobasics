...
if(navigator.geolocation){
    navigator.geolocation.getCurrentPosition(position => {
        ...
        AppelAPI(long,lat);
    },() => alert(`Vous avez refusé la géocalisation, l'application ne peut fonctionner, veuillez l'activer !`) )
}
...
function AppelAPI (long, lat){
    ...
    .then((data)=> {
        ...
        let heurActuelle = new Date().getHours();
        for (let i = 0; i< heure.length; i++) {
            let heureIncr= heurActuelle + i * 3;
        ...
        // Icone dynamique 
        if( heurActuelle >= 6 && heurActuelle < 21) {
